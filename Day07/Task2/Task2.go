...
func createCombinaisonFor(length int) []string {
	...
	for i := 0; i < totalCombinations; i++ {
		...
		num := i
		for j := 0; j < length; j++ {
			group.WriteString(operators[num % 3])
			num /= 3
		}
...
func isEquationPossible(result int, numbers []int) bool {
	...
	for _, combination := range combinations {
        ...
        for index, action := range combination {
			...
			if (action == '^') {
				tmpResultStr := fmt.Sprintf("%d%d", tmpResult, numbers[index + 1])
				tmpResult, _ = strconv.Atoi(tmpResultStr)
			}
...
func main() {
    ...
    // * Answer should be 275791737999003
}
